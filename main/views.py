...
def decode_jwt_with_jwks(token):
    ...
    try:
        if JWKS_URL:
            ...
            decoded = jwt.decode(
                token, signing_key.key, algorithms=["RS256"], issuer="Pandacare"
            )
            if (
                "roles" in decoded
                and isinstance(decoded["roles"], list)
                and decoded["roles"]
            ):
    ...
    if manual_decoded:
        ...
        if manual_decoded.get(
            "iss"
        ) != "Pandacare" or current_time > manual_decoded.get("exp", 0):
            return None
...
class CaregiverDashboardView(View):
    ...
    def _validate_session(self, request, required_role):
        return (
            is_logged_in(request) and request.session.get("user_role") == required_role
        )
    ...
    def _build_context(self, user_id, token):
        ...
        return {
            ...
            "pending_requests": waiting_reservations[:10]
    ...
    def _get_reservations(self, user_id, status, token):
        try:
            ...
            reservations = api_request(
                "GET", endpoint, params={"status": status}, token=token
            )
            ...
            if isinstance(reservations, list):
                ...
                if "data" in reservations:
                    return (
                        reservations["data"]
                        if isinstance(reservations["data"], list)
                        else []
                    )
                elif "reservations" in reservations:
                    return (
                        reservations["reservations"]
                        if isinstance(reservations["reservations"], list)
                        else []
                    )
...
class LoginView(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            access_token = (
                response.get("access") if isinstance(response, dict) else None
            )

            refresh_token = (
                response.get("refresh") if isinstance(response, dict) else None
            )
            ...
            if "unauthorized" in error_str or "401" in error_str:
                ...
                messages.error(
                    request, "Login failed. Please try again or contact support."
                )
    ...
    def post(self, request):
        ...
        try:
            ...
            login_response = api_request(
                "POST",
                "/api/auth/login",
                {"email": profile_data["email"], "password": profile_data["password"]},
            )
            ...
            for endpoint in profile_endpoints:
                try:
                    profile_response = api_request(
                        "POST", endpoint, profile_payload, token=access_token
                    )
            ...
            messages.error(
                request, "Unable to connect to server. Please try again later."
            )
            ...
            if "email" in error_str and (
                "exist" in error_str or "duplicate" in error_str
            ):
                messages.error(
                    request,
                    "Email already registered. Please use a different email or try logging in.",
                )
            elif "nik" in error_str and (
                "exist" in error_str or "duplicate" in error_str
            ):
                messages.error(
                    request,
                    "NIK already registered. Please check your NIK or contact support.",
                )
