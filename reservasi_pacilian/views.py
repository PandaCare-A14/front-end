...
def list_reservasi(request, id_pacilian):
    ...
    try:
        ...
        # Log the API error for debugging but don't fall back to mock data
...
# GET/POST: Edit reservation
def edit_reservasi(request, id):
    if not is_logged_in(request):
        messages.error(request, "Please login first")
        return redirect("main:login")
    ...
    if request.method == "POST":
        new_schedule_id = request.POST.get("idSchedule")
        
        if not new_schedule_id:
            messages.error(request, "New schedule is required")
            return render(request, "edit.html", {"idReservasi": id, **user_context})
        
        data = {"idSchedule": new_schedule_id}
        ...
        try:
            ...
            return redirect("pacilian_reservasi_list", id_pacilian=user_id)
            
        except Exception as e:
            messages.error(request, f"Failed to edit reservation: {str(e)}")
            return render(request, "edit.html", {"idReservasi": id, **user_context})
...
# POST: Test authentication flow
def test_auth_flow(request):
    # Simulate logged-in user session for testing
    test_token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9.eyJpc3MiOiJQYW5kYWNhcmUiLCJleHAiOjE3NDgxMzkwODksIm5iZiI6MTc0ODE2Mzg3OCwiaWF0IjoxNzQ4MTM4Nzg5LCJqdGkiOiIzNmMzNDg3My1iNGMzLTRhY2UtYWU2My0xZTI4NzBjMGFiMDIiLCJ1c2VyX2lkIjoiMTlhZWRlNzQtYzMwMi00ZWVhLWI1NmMtMGVlZGMxZDM0ZTNkIiwicm9sZXMiOlsicGFjaWxpYW4iXX0.lSYxOs1d3i4WSyAyqiMq_JsJ1XIbZw7S-VWsPPAOuPUR7FsI6pbwZ9RYQPcO2cayQvwDJPBbEtzHDloWZZfVO5o-lrPluQJZHLsIPVAXXGqHmKpz_6aMdv7M32KVswuw6Zt_n25xq3zuSqa-MHW4QQL2oz-vK8M9Aod3ebjNc3QH84Xvm0rYhD5E6NXHfyULRqhb7Uf8rVXlR7McDuMQvIrwgzn2WT4tXVKK1BLEcYHLDDsieEAOq7DBKRy4vLXxzy6hppaJLMzvXyvt43T8mpxm1dv_McGDub8gvzwjLWMca5OJ_RLkX3DuPJJTdxFk7Q0LwvWW4KF6ZXQDRs5S8rJTw"
    test_user_id = "19aede74-c302-4eea-b56c-0eedc1d34e3d"
    
    # Set session for testing
    request.session["access_token"] = test_token
    request.session["user_id"] = test_user_id
    request.session["user_role"] = "pacilian"
    
    # Now call the main function
    return list_reservasi(request, test_user_id)
